    ...
    <div x-data="{ expand: false }" class="col-span-3">
        ...
        <div x-show="!expand" @close.window="expand=false" class="flex justify-between gap-2">
            ...
            @if ($question->members->count())
                <x-ts-button @click="expand=true" xs flat primary icon="pencil" />
                <x-ts-button wire:click="removeAnswer" xs flat negative icon="trash" />
                {{-- <span wire:click="removeAnswer({{ $question->id }})">REMOVE</span> --}}
            @else
                <x-ts-button @click="expand=true" flat sm primary label="Adicionar resposta" class="-my-2" />
            @endif
        </div>
        ...
        <div x-show="expand" class="flex">
            ...
            <div>
                <x-ts-button wire:click="submitAnswer" sm flat primary icon="check" />
                <x-ts-button @click="expand=false" wire:click="resetInput" sm flat icon="x-mark" />
            </div>
        </div>
