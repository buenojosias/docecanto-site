<div>
    <x-ts-card header="Músicas" scope="without-padding">
        <x-card-header title="Músicas">
            <x-slot:action>
                <x-ts-button wire:click="openFormModal" icon="fluentui.add-12-o" sm flat />
            </x-slot:action>
        </x-card-header>
        <ul>
            @forelse ($songs as $song)
                <li class="border-b px-2">
                    <div class="flex items-center py-2">
                        ...
                        @can('coordinator')
                            <div>
                                <x-ts-button wire:click="removeSong({{ $song->id }})" negative sm flat icon="trash" />
                            </div>
                        @endcan
                    </div>
                    @if ($song->pivot->comment)
                        <div class="-mt-1 pb-2 pl-1 flex space-x-2 text-gray-700">
                            <x-ts-icon name="fluentui.comment-20-o" class="w-4" />
