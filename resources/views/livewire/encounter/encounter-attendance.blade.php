        ...
        <div class="card mb-4">
            <form wire:submit="submitAttendance">
                        ...
                        <tbody>
                            @foreach ($membersWithoutAttendence->where('status', 'Ativo') as $key => $member)
                                <tr>
                                    <td>{{ $member->name }}</td>
                                    <td class="w-4 px-0">
                                        <x-ts-radio :id="'attendance_' . $key . '_P'"
                                            name="selectAttendance[{{ $member->id }}][attendance]" label="P"
                                            value="P" wire:model="selectedAttendance.{{ $member->id }}" />
                                    </td>
                                    <td class="w-4 px-0">
                                        <x-ts-radio :id="'attendance_' . $key . '_F'"
                                            name="selectAttendance[{{ $member->id }}][attendance]" label="F"
                                            value="F" wire:model="selectedAttendance.{{ $member->id }}" />
                                    </td>
                                    <td class="w-4 px-0">
                                        <x-ts-radio :id="'attendance_' . $key . '_J'"
                                            name="selectAttendance[{{ $member->id }}][attendance]" label="J"
                                            value="J" wire:model="selectedAttendance.{{ $member->id }}" />
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                ...
                <div class="card-footer space-x-2">
                    <x-ts-button type="submit" primary sm text="Salvar" loading="submitAttendance" />
                        ...
                        <tr>
                            <td x-data="{ showNote: false }">
                                ...
                                @if ($member->pivot->note)
                                    <x-ts-button @click="showNote = !showNote" xs flat primary
                            ...
                            @can('coordinator')
                                <td class="w-8">
                                    <x-ts-button wire:click="openChangeModal({{ $member }})" flat xs
                                        text="Alterar" />
                                </td>
                            @endcan
                        </tr>
